...
function RulingsSheet({ onClose, closing }) {
  ...
  const [card, setCard]           = useState(null);    // selected card object
  const [rulings, setRulings]     = useState(null);    // array of ruling objects
  const [cardLoading, setCardLoad]= useState(false);
  ...
  const onQueryChange = (val) => {
    ...
    if (val.length < 2) { setSugs([]); setShowSugs(false); return; }
  ...
  const handleKey = (e) => {
    if (e.key === "Enter" && query.trim()) {
      const exact = suggestions[0] || query.trim();
      selectCard(exact);
    }
  ...
  const clearCard = () => {
    setCard(null); setRulings(null); setQuery(""); setSugs([]); setError(null);
  };
  ...
  const faceText = (c) =>
    c.oracle_text || (c.card_faces && c.card_faces.map(f => f.name + "\n" + (f.oracle_text||"")).join("\n---\n")) || "";

  const formatPrice = (val) => val ? "$" + parseFloat(val).toFixed(2) : null;
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {/* Drag pill */}
        <div style={{ width: 36, height: 4, borderRadius: 2, background: COLORS.border, margin: "14px auto 8px", flexShrink: 0 }} />

        {/* Header */}
        <div style={{
          padding: "0 16px 12px", borderBottom: `1px solid ${COLORS.border}`,
          ...
          <div style={{ flex: 1, position: "relative" }}>
            ...
            />
            {sugLoading && (
              <span style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: COLORS.muted, fontSize: 12 }}>⟳</span>
            )}
            ...
            {showSugs && suggestions.length > 0 && (
              <div style={{
                ...
                borderRadius: 10, zIndex: 10, maxHeight: 220, overflowY: "auto",
        ...
        <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
          ...
          {!card && !cardLoading && !error && (
            <div style={{ textAlign: "center", padding: "50px 24px", color: COLORS.muted }}>
              ...
              <div style={{ fontSize: 14, lineHeight: 1.8 }}>Search any Magic card<br/>to see rulings and prices.</div>
            </div>
          )}

          {/* Loading */}
          {cardLoading && (
            <div style={{ textAlign: "center", padding: "50px 24px", color: COLORS.muted, fontSize: 14 }}>
              ⟳ Loading card…
            </div>
          )}
          ...
          {/* Error */}
          {error && (
            <div style={{ textAlign: "center", padding: "40px 24px", color: COLORS.red, fontSize: 14 }}>
              {error}
            </div>
          )}
          ...
          {card && !cardLoading && (() => {
            ...
            const txt = faceText(card);
            const prices = card.prices || {};
            const usd     = formatPrice(prices.usd);
            const usdFoil = formatPrice(prices.usd_foil);
            const eur     = formatPrice(prices.eur);
            const eurFoil = formatPrice(prices.eur_foil);
            const hasPrices = usd || usdFoil || eur || eurFoil;
            ...
            return (
              <div style={{ padding: "16px 16px 40px" }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, marginBottom: 4 }}>
                  <div style={{ fontSize: 18, fontWeight: "bold", color: COLORS.text }}>{card.name}</div>
                  <div style={{ fontSize: 14, color: COLORS.muted, flexShrink: 0 }}>{card.mana_cost}</div>
                </div>

                {/* Type line */}
                <div style={{ fontSize: 12, color: COLORS.muted, marginBottom: 10, fontStyle: "italic" }}>
                  {card.type_line}
                  {card.power != null && (
                  ...
                  {card.loyalty != null && (
                    <span style={{ marginLeft: 8, color: COLORS.eot, fontStyle: "normal", fontWeight: "bold" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {usd && (
                        <div style={{
                          ...
                          <div style={{ fontSize: 10, color: COLORS.muted }}>USD</div>
                          <div style={{ fontSize: 15, fontWeight: "bold", color: COLORS.teal }}>{usd}</div>
                        </div>
                      )}
                      {usdFoil && (
                        <div style={{
                          ...
                          <div style={{ fontSize: 10, color: COLORS.muted }}>USD Foil</div>
                          <div style={{ fontSize: 15, fontWeight: "bold", color: COLORS.gold }}>{usdFoil}</div>
                        </div>
                      )}
                      {eur && (
                        <div style={{
                          ...
                          <div style={{ fontSize: 10, color: COLORS.muted }}>EUR</div>
                          <div style={{ fontSize: 15, fontWeight: "bold", color: COLORS.eot }}>{eur}</div>
                        </div>
                      )}
                      {eurFoil && (
                        <div style={{
                          ...
                          <div style={{ fontSize: 10, color: COLORS.muted }}>EUR Foil</div>
                          <div style={{ fontSize: 15, fontWeight: "bold", color: COLORS.eot }}>{eurFoil}</div>
                        </div>
                      )}
                    </div>
                    <div style={{ fontSize: 10, color: COLORS.border, marginTop: 6 }}>
                ...
                {/* Rulings */}
                <div>
                  <div style={{ fontSize: 10, color: COLORS.muted, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>
                    ⚖ Official Rulings {rulings ? `(${rulings.length})` : ""}
                  </div>
                  {rulings === null && (
                    <div style={{ color: COLORS.muted, fontSize: 13 }}>Loading rulings…</div>
                  )}
                  {rulings && rulings.length === 0 && (
                    <div style={{ color: COLORS.muted, fontSize: 13, fontStyle: "italic" }}>No rulings on record for this card.</div>
                  )}
                  {rulings && rulings.map((r, i) => (
                    <div key={i} style={{
                      ...
                      <div style={{ fontSize: 10, color: COLORS.muted, marginBottom: 4 }}>{r.published_at}</div>
                      <div style={{ fontSize: 13, color: COLORS.text, lineHeight: 1.6 }}>{r.comment}</div>
                    </div>
                  ))}
                </div>
              </div>
...
window.__MTGApp = App;const { useState, useCallback, useRef } = React;
